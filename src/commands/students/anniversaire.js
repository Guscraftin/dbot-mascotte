...
module.exports = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("date")
            .setDescription("Ta date de naissance (DD/MM/AAAA).")
            .setMinLength(10)
            .setMaxLength(10)
            .setRequired(true)
        )
      ...
      subcommand
        ...
        .addBooleanOption((option) =>
          option
            ...
            .setDescription(
              "Confirmer la suppression de votre date de naissance."
            )
            .setRequired(true)
        )
    ),
  async execute(interaction) {
    ...
    if (date) {
      ...
      if (
        dateFormat.getDate() != day ||
        dateFormat.getMonth() + 1 != month ||
        dateFormat.getFullYear() != year
      )
        return interaction.reply({
          content: `La date entrée est invalide \`${date}\`.`,
          ephemeral: true,
        });
      if (dateFormat > new Date())
        return interaction.reply({
          content: `La date entrée est invalide \`${date}\`.\nVeuillez saisir votre date de naissance en réutilisant cette commande.`,
          ephemeral: true,
        });
    ...
    switch (interaction.options.getSubcommand()) {
       ...
       */
      case "ajouter":
        if (member && member.date_birthday)
          return interaction.reply({
            content: "Vous avez déjà ajouté votre date de naissance.",
            ephemeral: true,
          });
        await Members.upsert(
          { id: interaction.member.id, date_birthday: dateFormat },
          { where: { id: interaction.member.id } }
        );
        ...
        await channelLog?.send({
          content: `## 🎂 ${
            interaction.member
          } a ajouté sa date de naissance.\n> Date de naissance : <t:${parseInt(
            dateFormat / 1000
          )}:D> (\`${
            nextBirthday.getFullYear() - dateFormat.getFullYear() - 1
          } ans\`)`,
        });
        ...
        return interaction.reply({
          content:
            "Votre date de naissance a correctement été ajouté.\nMerci de votre investissement sur ce serveur discord.",
          ephemeral: true,
        });
      ...
      case "liste":
        ...
        if (!members.length)
          return interaction.reply({
            content: "Aucun utilisateur n'a encore participé.",
            ephemeral: true,
          });
          ...
          .setDescription(
            birthdayPage
              .map((user) => {
                ...
                return `<t:${parseInt(nextBirthday / 1000)}:D> ▸ <@${
                  user.id
                }> (\`${
                  nextBirthday.getFullYear() -
                  user.date_birthday.getFullYear() -
                  1
                } ans\`)`;
              })
              .join("\n")
      ...
      case "modifier":
        if (!member || !member.date_birthday)
          return interaction.reply({
            content: "Vous n'avez pas ajouté votre date de naissance.",
            ephemeral: true,
          });
        await member.update({ date_birthday: dateFormat });

        oldNextBirthday = new Date(
          actualYear,
          oldDate.getMonth(),
          oldDate.getDate()
        );
        if (oldNextBirthday < new Date())
          oldNextBirthday.setFullYear(actualYear + 1);
        await channelLog?.send({
          content: `## 🎂 ${
            interaction.member
          } a modifié sa date de naissance.\n> Ancienne date de naissance : <t:${parseInt(
            oldDate / 1000
          )}:D> (\`${
            oldNextBirthday.getFullYear() - oldDate.getFullYear() - 1
          } ans\`)\n> Nouvelle date de naissance : <t:${parseInt(
            dateFormat / 1000
          )}:D> (\`${
            nextBirthday.getFullYear() - dateFormat.getFullYear() - 1
          } ans\`)`,
        ...
        return interaction.reply({
          content:
            "Votre date de naissance a correctement été modifié.\nMerci de votre investissement sur ce serveur discord.",
          ephemeral: true,
        });
      ...
      case "supprimer":
        ...
        if (!confirmation)
          return interaction.reply({
            content:
              "Vous devez confirmer la suppression de votre date de naissance.",
            ephemeral: true,
          });
        ...
        await channelLog?.send({
          content: `## 🎂 ${
            interaction.member
          } a supprimé sa date de naissance.\n> Date de naissance : <t:${parseInt(
            oldDate / 1000
          )}:D> (\`${
            oldNextBirthday.getFullYear() - oldDate.getFullYear() - 1
          } ans\`)`,
        ...
        return interaction.reply({
          content:
            "Votre date de naissance a correctement été supprimé.\nMerci de votre investissement sur ce serveur discord.",
          ephemeral: true,
        });
