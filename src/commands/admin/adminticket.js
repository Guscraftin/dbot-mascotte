...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("adminticket")
        .setDescription("🔧 Permet de modifier les retranscriptions des tickets supprimés.")
    ...
    async execute(interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "list":
                ...
                try {
                    if (member && category) tickets = await Tickets.findAll({ where: { user_id: member.id, category: category }, order: [['updatedAt', 'DESC']] });
                    else if (member) tickets = await Tickets.findAll({ where: { user_id: member.id }, order: [['updatedAt', 'DESC']] });
                    else if (category) tickets = await Tickets.findAll({ where: { category: category }, order: [['updatedAt', 'DESC']] });
                    else tickets = await Tickets.findAll({ where: { channel_id: null }, order: [['updatedAt', 'DESC']] });
                } catch (error) {
                    console.error("adminticket list - " + error);
                }
                if (!tickets || tickets.length == 0) return interaction.editReply({ content: `Aucune retranscription de ticket n'a été trouvée.`, ephemeral: true });
