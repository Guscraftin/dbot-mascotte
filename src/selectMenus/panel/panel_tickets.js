  ...
  async execute(interaction) {
    ...
    switch (interaction.values[0]) {
      ...
      case "ticket_admins":
        ...
        if (ticket)
          return interaction.editReply({
            content: `Vous avez déjà un ticket d'ouvert dans <#${ticket.channel_id}> !`,
            ephemeral: true,
          });
      ...
      case "ticket_delegues":
        ...
        if (!interaction.member.roles.cache.has(role_students))
          return interaction.editReply({
            content:
              "Vous n'avez pas la possibilité de contacter directement les délégués. Pour signaler le problème, veuillez ouvrir un ticket auprès des admins.",
            ephemeral: true,
          });

        // Check if there are at least one delegate
        const delegates = await interaction.guild.roles
          .fetch(role_delegates)
          .then((role) => role.members);
        if (delegates.size === 0)
          return interaction.editReply({
            content:
              "Il n'y a pas de délégués pour le moment. Veuillez contacter un admin.",
            ephemeral: true,
          });
      ...
      case "ticket_exit":
        return interaction.editReply({
          content: "Vous avez annulé la création d'un ticket !",
          ephemeral: true,
        });
      ...
      default:
        return interaction.editReply({
          content: "Vous n'avez pas choisi d'option !",
          ephemeral: true,
        });
    }
    ...
    message.pin();
    const msgMention = await channel.send({
      content: `<@${interaction.user.id}>, vous avez un nouveau ticket avec les <@&${roleSupport}> !`,
    });
    msgMention.delete();
