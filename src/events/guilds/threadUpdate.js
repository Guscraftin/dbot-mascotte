...
module.exports = {
  ...
  async execute(oldThread, newThread) {
    ...
    const embedModif = new EmbedBuilder()
      ...
      .setDescription(
        `Le thread ${
          oldThread.type === ChannelType.PublicThread ? `public` : `privé`
        } <#${oldThread.id}> (\`${
          oldThread.name
        }\`) **a été modifié** dans le salon ${
          newThread.parent
        } par un modérateur !
            ${
              oldThread.name != newThread.name
                ? `> **Nom :** \`${oldThread.name}\` => \`${newThread.name}\`\n`
                : ``
            } ${
          oldThread.rateLimitPerUser != newThread.rateLimitPerUser
            ? `> **Mode lent :** \`${resultRateLimit(
                oldThread
              )}\` => \`${resultRateLimit(newThread)}\`\n`
            : ``
        } ${
          oldThread.autoArchiveDuration != newThread.autoArchiveDuration
            ? `> **Archivage après une période d'inactivité :** \`${resultAutoArchive(
                oldThread
              )}\` => \`${resultAutoArchive(newThread)}\`\n`
            : ``
        } ${
          !oldThread.locked && newThread.locked
            ? `> **Verouillage :** \`Désactivé\` => \`Activé\``
            : ``
        } ${
          oldThread.locked && !newThread.locked
            ? `> **Verouillage :** \`Activé\` => \`Désactivé\``
            : ``
        }
            `
      )
    ...
    const embedArchiv = new EmbedBuilder()
      ...
      .setDescription(
        `Le thread ${
          ...
          oldThread.name
        }\`) a été **archivé** dans le salon ${newThread.parent} ${
          !oldThread.locked && newThread.locked ? `par un __modérateur__` : ``
        } !
            `
      )
      .setTimestamp()
    ...
    const embedDeArchiv = new EmbedBuilder()
      ...
      .setDescription(
        `Le thread ${
          ...
          oldThread.name
        }\`) a été **déarchivé** dans le salon ${newThread.parent} !
    ...
    if (!oldThread.archived && newThread.archived)
      logChannel?.send({ embeds: [embedArchiv] });
