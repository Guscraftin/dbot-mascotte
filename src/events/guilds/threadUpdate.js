...
module.exports = {
    ...
    async execute(oldThread, newThread){
        ...
        const embedModif = new EmbedBuilder()
            ...
            .setDescription(`Le thread ${oldThread.type === ChannelType.PublicThread ? `public` : `privé`} <#${oldThread.id}> (\`${oldThread.name}\`) **a été modifié** dans le salon ${newThread.parent} par un modérateur !
        ...
        const embedArchiv = new EmbedBuilder()
            ...
            .setColor(color_basic)
            .setDescription(`Le thread ${oldThread.type === ChannelType.PublicThread ? `public` : `privé`} <#${oldThread.id}> (\`${oldThread.name}\`) a été **archivé** dans le salon ${newThread.parent} ${!oldThread.locked && newThread.locked ? `par un __modérateur__` : ``} !
            `)
            .setTimestamp()
            .setFooter({ text: newThread.guild.name, iconURL: newThread.guild.iconURL() })
        ...
        const embedDeArchiv = new EmbedBuilder()
            ...
            .setColor(color_basic)
            .setDescription(`Le thread ${oldThread.type === ChannelType.PublicThread ? `public` : `privé`} <#${oldThread.id}> (\`${oldThread.name}\`) a été **déarchivé** dans le salon ${newThread.parent} !
