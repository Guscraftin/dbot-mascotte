  ...
  async execute(interaction) {
    if (interaction.isCommand()) {
      ...
      try {
        ...
        console.error(
          `Erreur d'exécution du boutton : ${interaction.customId}`
        );
        console.error(error);
      }
      ...
      if (!selectMenu)
        return new Error("Il n'y a pas de code pour ce menu déroulant.");

      try {
        await selectMenu.execute(interaction);
      } catch (error) {
        console.error(
          `Erreur d'exécution du menu déroulant : ${interaction.customId}`
        );
        console.error(error);
      }
      ...
      try {
        ...
        console.error(
          `Erreur d'exécution de la modal : ${interaction.customId}`
        );
        console.error(error);
      }
      ...
      if (!contextMenu)
        return new Error("Il n'y a pas de code pour ce menu contextuel.");

      try {
        await contextMenu.execute(interaction);
      } catch (error) {
        console.error(
          `Erreur d'exécution du menu contextuel : ${interaction.customId}`
        );
        console.error(error);
      }
    } else {
