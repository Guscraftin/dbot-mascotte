...
module.exports = {
  ...
  async execute(oldStageInstance, newStageInstance) {
     ...
     * Logs the event
     */
    const logChannel = await newStageInstance.guild.channels.fetch(
      channel_logs
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**La conférence \`${newStageInstance.topic}\` a été modifiée dans le salon ${newStageInstance.channel}.**
            > **Sujet :** \`${oldStageInstance.topic}\` => \`${newStageInstance.topic}\`
            `
      )
      .setTimestamp()
      .setFooter({
        text: newStageInstance.guild.name,
