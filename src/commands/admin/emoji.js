...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("id")
        .setDescription("L'id du message où ajouter les réactions.")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("emojis")
        .setDescription("Les emojis à ajouter dans l'ordre.")
        .setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (!onlyNumber.test(messageId))
      return interaction.editReply({
        content: "L'id doit être l'identifiant d'un message dans ce salon.",
        ephemeral: true,
      });

    // Filter the emojis
    const msgEmojiRegex = new RegExp(
      "(<:[A-Za-z0-9_]+:[0-9]+>)|(" + emojiRegex() + ")",
      "gu"
    );
    ...
    if (isMax)
      return interaction.editReply({
        content: `⚠️ **Le nombre de réactions est limité à 20 par message.** Les \`${emojisDeleted.length}\` derniers emojis n'ont pas pu être ajoutés. Cela correspond aux emojis suivants : ${emojisDeleted}.`,
      });
    else
      return interaction.editReply({
        content: `Les emojis ont bien été ajoutés au message.`,
        ephemeral: true,
      });
  },
};
