...
const { category_reserve, channel_logs_tickets, role_admins } = require(process.env.CONSTANT);
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const promises = await channel.permissionOverwrites.cache.map(async overwrite => {
            if (overwrite.id !== interaction.guild.id) await overwrite.edit({ ViewChannel: false });
        });
        ...
        await channel.send({ content: `## Seul les <@&${role_admins}> on accès à ce ticket.`, components: [deleteTicket] });
        ...
        const msg = await channelLogs.send({
            content: `**Le ticket de ${ticket?.user_id ? `<@${ticket.user_id}>` : `\`${channelNameSplit.slice(1).join("-")}\``} a été fermé par ${interaction.user}.${ticket?.id ? ` (\`#${ticket.id}\`)` : ``}**\n**Catégorie :** \`${ticket?.category}\`\n**Date d'ouverture du ticket :** <t:${parseInt(channel.createdAt / 1000)}:F> (<t:${parseInt(channel.createdAt / 1000)}:R>)\n**Date de fermeture du ticket :** <t:${parseInt(Date.now() / 1000)}:F> (<t:${parseInt(Date.now() / 1000)}:R>)`,
            files: [attachment]
        });
