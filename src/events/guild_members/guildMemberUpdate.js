...
module.exports = {
  ...
  async execute(oldMember, newMember) {
     ...
     */
    if (
      !oldMember.roles.cache.has(role_class) &&
      newMember.roles.cache.has(role_class)
    ) {
    ...
    if (logChannel) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${newMember.user} a été mis à jour.**
            ${
              oldMember.displayName !== newMember.displayName
                ? `> **Surnom :** \`${oldMember.displayName}\` => \`${newMember.displayName}\`\n`
                : ``
            }${
            addRoles.length !== 0 ? `> **Rôle ajouté :** ${addRoles}\n` : ``
          }${
            removeRoles.length !== 0
              ? `> **Rôle supprimé :** ${removeRoles}\n`
              : ``
          }`
        )
        .setTimestamp()
