...
module.exports = {
  ...
  async execute(messageReaction, user) {
     ...
     * Logs the event
     */
    const logChannel = await messageReaction.message.guild.channels.fetch(
      channel_logs
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**<@${user.id}> a retiré sa réaction ${
          isDefaultEmoji()
            ? `\`${messageReaction.emoji.name}\``
            : `<:${messageReaction.emoji.name}:${messageReaction.emoji.id}>`
        } [à ce message](${messageReaction.message.url}).**
            `
      )
      .setTimestamp()
      .setFooter({
        text: messageReaction.message.guild.name,
        iconURL: messageReaction.message.guild.iconURL(),
      });
