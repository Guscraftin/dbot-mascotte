...
module.exports = {
  ...
  async execute(message) {
    ...
    if (message.channel.id === channel_idea_poll && !message.author.bot) {
      ...
      const emojiArrayFiltered = emojiArray.filter(
        (emoji) => emoji !== "💬" && emoji !== "🗑️"
      );
      ...
      if (emojiArrayFiltered.length > 0) {
        ...
        const embedPoll = new EmbedBuilder()
          .setAuthor({
            name: `${message.member.displayName} (${message.author.id})`,
        ...
        if (emojiArrayFiltered.length > 18) {
          ...
          await message.author.send({
            content: `⚠️ **Le nombre de réactions est limité à 18 par sondage dans <#${
              message.channel.id
            }>.** Les \`${
              emojiArrayFiltered.length - 18
            }\` dernières réactions de ton message n'ont pas pu être ajoutées.\nSi le message ne te convient pas, tu peux le supprimer en cliquant sur l'emoji \`🗑️\` situé en dessous.`,
          });
        }
      ...
      if (guild) {
        if (
          guild.automatic_mention_idea_poll &&
          !guild.blacklist_mention_idea_poll.includes(message.author.id)
        ) {
          const notificationMsg = await message.channel.send({
            content: `<@&${role_idea_poll}>`,
          });
