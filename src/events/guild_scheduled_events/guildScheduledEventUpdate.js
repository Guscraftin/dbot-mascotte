...
module.exports = {
  ...
  async execute(oldGuildScheduledEvent, newGuildScheduledEvent) {
     ...
     * Logs the event
     */
    const logChannel = await newGuildScheduledEvent.guild.channels.fetch(
      channel_logs
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `${user} a **mise à jour** l'événement nommé \`${newGuildScheduledEvent.name}\`
            `
      )
      .setTimestamp()
      .setFooter({
        text: newGuildScheduledEvent.guild.name,
