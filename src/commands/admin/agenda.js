...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("agenda")
        .setDescription("🔧 Permet de récupérer des informations sur un repo github.")
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .setDMPermission(true)
        .addStringOption(option => option.setName('group').setDescription("Recevoir l\'emploi du temps dans ce salon tous les jours à 18h.").setRequired(true)),
    async execute(interaction) {
        ...
        if(!group)
           return interaction.reply('La classe est invalide ou introuvable (ex: S1S2 B2)');

        const groupSearch = await fetch(`https://zeus.ionis-it.com/api/group/${group.id}/ics`, {
            method: "GET",
            headers: {
                'Authorization': process.env.ZEUS_TOKEN
            }
        ...
        // Return the message
        return interaction.reply(`L'emploi du temps de la classe **${group.name}** a été trouvé.`);
