...
module.exports = {
  ...
  async execute(oldState, newState) {
    ...
    if (newState && newState.channelId === vocal_panel) {
      ...
      const voiceChannel = await newState.channel.clone({
    ...
    if (
      oldChannel?.parentId === category_vocals &&
      oldChannel?.members?.size === 0 &&
      !channelsNotDelete.includes(oldState.channelId)
    ) {
    ...
    if (logChannel) {
      if (oldState.channel === null) {
        const embedJoin = new EmbedBuilder()
          ...
          .setDescription(
            `<@${newState.member.id}> **a rejoint le salon vocal ${newState.channel} ||(\`${newState?.channel?.name}\`)||.**
                `
          )
          .setTimestamp()
          .setFooter({
        ...
        const embedLeave = new EmbedBuilder()
          ...
          .setColor(color_basic)
          .setDescription(
            `<@${oldState.member.id}> **a quitté le salon vocal ${oldState.channel} ||(\`${oldState.channel.name}\`)||.**
                `
          )
          .setTimestamp()
          .setFooter({
        ...
        const embedMove = new EmbedBuilder()
          ...
          .setColor(color_basic)
          .setDescription(
            `<@${oldState.member.id}> **a changé de salon vocal ${oldState.channel} => ${newState.channel}.**
        ...
        const embedDeaf = new EmbedBuilder()
          ...
          .setColor(color_basic)
          .setDescription(
            `**Le statut du micro de <@${
              oldState.member.id
            }> à été mis à jour dans le salon vocal ${oldState.channel} ||(\`${
              oldState.channel.name
            }\`)||.**
                ${
                  !oldState.selfMute && newState.selfMute
                    ? `> 🎙 **Muet :** \`Désactivé\` => \`Activé\`\n`
                    : ``
                } ${
              oldState.selfMute && !newState.selfMute
              ...
              !oldState.selfDeaf && newState.selfDeaf
                ? `> 🔈 **Sourd :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.selfDeaf && !newState.selfDeaf
                ? `> 🔈 **Sourd :** \`Activé\` => \`Désactivé\`\n`
                : ``
            } ${
              !oldState.serverMute && newState.serverMute
                ? `> 🎙 **Muet par un modo :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.serverMute && !newState.serverMute
                ? `> 🎙 **Muet par un modo :** \`Activé\` => \`Désactivé\`\n`
                : ``
            } ${
              !oldState.serverDeaf && newState.serverDeaf
                ? `> 🔈 **Sourd par un modo :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.serverDeaf && !newState.serverDeaf
                ? `> 🔈 **Sourd par un modo :** \`Activé\` => \`Désactivé\`\n`
                : ``
            } ${
              !oldState.selfVideo && newState.selfVideo
                ? `> 📷 **Camera :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.selfVideo && !newState.selfVideo
                ? `> 📷 **Camera :** \`Activé\` => \`Désactivé\`\n`
                : ``
            } ${
              !oldState.streaming && newState.streaming
                ? `> 📺 **Partage d'écran :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.streaming && !newState.streaming
                ? `> 📺 **Partage d'écran :** \`Activé\` => \`Désactivé\`\n`
                : ``
            } ${
              !oldState.suppress && newState.suppress
                ? `> 📣 **Muet dans Stage :** \`Désactivé\` => \`Activé\`\n`
                : ``
            } ${
              oldState.suppress && !newState.suppress
                ? `> 📣 **Muet dans Stage :** \`Activé\` => \`Désactivé\`\n`
                : ``
            }
                `
          )
