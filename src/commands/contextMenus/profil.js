  ...
  async execute(interaction) {
    ...
    if (user) {
      ...
      if (user.date_birthday) {
        ...
        fields.push({
          name: "Anniversaire",
          value: `Date de naissance : <t:${parseInt(
            user.date_birthday / 1000
          )}:D> (\`${
            nextBirthday.getFullYear() - dateBirthday.getFullYear() - 1
          } ans\`)\nProchain anniversaire <t:${parseInt(
            nextBirthday / 1000
          )}:R>`,
          inline: true,
        });
      ...
      if (user.mute_time)
        fields.push({
          ...
          value: `Se termine <t:${parseInt(user.mute_time / 1000)}:R>`,
          inline: true,
        });
      ...
      embed.setDescription(
        `Aucune information n'est disponible sur ce membre.`
      );
    }
